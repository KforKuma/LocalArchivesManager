    ...
    def run(self, *, dry_run: bool = False) -> WorkflowResult:
        ...
        for record in records:
            ...
            try:
                ...
                legacy = "legacy_topic_location" in str(exc)
                outcome = catalogue.ensure_review_blocker(
                    record,
                    "pdf_relative_path" if legacy else "pdf_file",
                    str(exc),
                    issue_key="legacy_topic_location" if legacy else "unsafe_source",
                    conflict_with_confirmation=True,
                )
                self._record_review(
                    result,
                    outcome,
                    {
                        ...
                        "file": relative,
                        "issue": "legacy_topic_location" if legacy else "unsafe_source",
                    },
                )
